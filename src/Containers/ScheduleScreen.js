...
import LimnEditor from "../Components/limn/editor";
import PreviewModal from "../Components/limn/utils/PreviewModal";
...
import CustomThemeInput from '../Components/limn/utils/CustomThemeInput';
...
const CUSTOM_ANIMATION_BUTTON_LABEL = "Custom animation";
...
class ScheduleScreen extends Component {
  ...
  constructor(props) {
    ...
    const tempImg = cookies.get("pageId")
      ? `http://graph.facebook.com/${cookies.get("pageId")}/picture?width=200`
      : noImage;
  ...
  changeBackground = () => {
    ...
    if (Object.keys(previewData).length) {
      if (previewData.animatedColors.length > 1) {
        // const currentColor = previewData.currentFrameData.color;
        const currentColorIndex = previewData.animatedColors.findIndex(
          color => color === previewData.currentFrameData.color
        );

        const nextColor =
          previewData.animatedColors[currentColorIndex + 1] ||
          previewData.animatedColors[0];
      ...
      if (
        previewData.animatedColors.length === 1 &&
        previewData.currentFrameData.color !== previewData.animatedColors[0]
      ) {
        this.setState({
          previewData: {
            ...previewData,
            currentFrameData: {
              color: previewData.animatedColors[0],
  ...
  loadEventData() {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      this.setState({
        ...
        selectedDatesDays: testData.reduce((acc, item) => {
      ...
      fetch("/scheduler/read", { credentials: "same-origin" })
        ...
        .then(eventData => {
          ...
          this.setState({
            selectedDates: sortedData,
            selectedDatesDays: sortedData.reduce((acc, item) => {
              return [...acc, item.date];
            }, []),
            upcoming: sortedData[0],
            upcomingNumber: sortedData.length
  ...
  setUpcoming() {
    this.setState(prev => {
      let tempDates = prev.selectedDates.filter(
        item => item.time > moment().valueOf()
      );
      tempDates.sort((a, b) => {
  ...
  renderCalendarTile(current, value) {
    ...
    if (index >= 0) {
      classNames =
        selectedDates[index].date > Date.now()
          ? "tile tile-busy"
          : "tile tile-expired";
      // classNames = 'tile  tile-busy'
      time = (
        <label className=" time_label">
          {moment(selectedDates[index].time).format(TIME_FORMAT)}
        </label>
      );

      if (indices.length === 1) {
        const shareTypeSvg =
          selectedDates[index].channel === FACEBOOK_POST.toUpperCase()
            ? FbPostLogoBlue
            : selectedDates[index].channel === FACEBOOK_MESSAGE.toUpperCase()
              ? FbMessageLogoBlue
              : "";

        shareType = (
          <div
            className=" share_type_label"
            style={{
              backgroundSize: "contain",
              backgroundImage: `url(${shareTypeSvg})`
            }}
          />
        );
      } else {
        shareType = (
          <div
            ...
            style={{
              backgroundSize: "contain",
              backgroundImage: `url(${FbPostLogoBlue})`
            }}
          />
        );
        shareTypeMix = (
          <div
            ...
            style={{
              marginLeft: "18px",
              backgroundSize: "contain",
              backgroundImage: `url(${FbMessageLogoBlue})`
            }}
          />
        );
      }
    ...
    if (current.format(DATE_FORMAT) === moment().format(DATE_FORMAT))
      classNames = "tile  tile-selected";
    return (
      <div className={classNames}>
        {/*marker*/} {time} {shareType} {shareTypeMix}
        <label style={{ margin: "0.25em", fontSize: "16px" }}>
          {current.date()}
        </label>
      </div>
    );
  ...
  renderShareTypePopover() {
    const logo =
      this.state.channel === FACEBOOK_MESSAGE.toUpperCase()
        ? FbMessageLogo
        : FbPostLogo;
    return (
      <div className="buttons_wrapper" key="buttons_wrapper">
        <FlatButton
          ...
          children={
            ...
            >
              <label
                key={"ShareTypeLabel"}
                style={{ marginLeft: "40px", color: "#fff", lineHeight: 1 }}
              >
        ...
        >
          ...
          >
            <MenuItem primaryText={FACEBOOK_MESSAGE} value={FACEBOOK_MESSAGE} />
            <MenuItem primaryText={FACEBOOK_POST} value={FACEBOOK_POST} />
          </Menu>
        </Popover>
      </div>
    );
  ...
  renderSelectedImage(product, key) {
    const imgStyle = {
      backgroundImage: `url(${product.image || this.state.noImageUrl})`
    };
    return (
  ...
  renderCustomImage() {
    return <div className="upload-btn-wrapper">
      <button className="btn">Upload custom file</button>
      <input type="file" name="file" onChange={this.handleUpload} multiple="true" />
    </div>
  }

  renderCustomImagePreview() {
    // return <div className="image-preview" style={{ backgroundColor: "green", height: '50px', width: '50px' }}>
    //   {/* Display all selected images. */}
    //   {
    //     this.state.customImage && <img src={URL.createObjectURL(this.state.customImage)} alt="file" style={{ backgroundColor: "green", height: '100%', width: '100%' }} />
    //   }
    // </div>


    const imgStyle = {
      backgroundImage: `url(${this.state.customImage ? URL.createObjectURL(this.state.customImage) : this.state.noImageUrl})`
    };
    return (
      this.state.customImage && <div className="image_container_modal">
        <div className="image" style={imgStyle} />
        <div className="remove-icon">
          <DeleteIcon
            color={REMOVE_COLOR}
            onClick={() => {
              this.setState({ customImage: null });
            }}
          />
        </div>
      </div>
  ...
  renderCustomAnimationButton() {
    ...
    return (
      <div style={{ clear: "both" }}>
        ...
        <FlatButton
          className="modal_popover image_set_button_margin"
          label={CUSTOM_ANIMATION_BUTTON_LABEL}
          onClick={() => this.openLimnEditor()}
          labelStyle={BUTTON_LABEL_STYLE}
        />
      </div>
    );
  ...
  customAnimationImage = data => {
    this.setState({
      previewData: {
        ...
        currentFrameData: data.currentFrameData,
  ...
  renderModal() {
    const { modalOpened, showLimnEditor, prodSelectorOpened } = this.state;
    const prodSelectorDisplay = prodSelectorOpened ? "inline-block" : "none";
    return (
      <div style={{ display: modalOpened ? "block" : "none" }}>
        {/* {showLimnEditor ? ( */}
        <div
          className=" highlight-container"
          style={{ display: prodSelectorDisplay }}
          onClick={this.closeModal}
        >
          <div
            className="prod_selector-container"
            style={{ display: prodSelectorDisplay }}
            onClick={e => {
              e.stopPropagation();
            }}
          >
            <LimnEditor
              customAnimationImage={this.customAnimationImage}
              closeLimnEditor={this.closeLimnEditor}
            />
          </div>
        </div>
        {/* ) : ( */}
        {!showLimnEditor ? this.renderModelContent() : null}
        {/* )} */}
      </div>
    );
  }

  onDeleteCustomAnimation = () => {
    this.setState({ previewData: {} });
  };

  renderSelectedCustomAnimation = () => {
    return Object.keys(this.state.previewData).length ? (
      <PreviewModal
        {...this.state.previewData}
        {...this.state.customImage}
        onDelete={this.onDeleteCustomAnimation}
      />
    ) : null;
  };
  ...
  renderModelContent() {
    ...
    const userUnselectDisplay =
      selectedSegmentValue === "" ? "none" : "inline-flex";

    const removeButton = selectedDatesDays.includes(selectedDate) ? (
      <FlatButton
        primary
        label={REMOVE}
    ...
    return (
      <div className=" highlight-container" onClick={this.closeModal}>
        <div
          className="prod_selector-container"
          style={{ display: prodSelectorDisplay }}
          onClick={e => {
        ...
        <div
          className=" action_selector_container "
          onClick={e => {
            e.stopPropagation();
          }}
        >
          <div style={{ paddingBottom: "1em" }}>
            ...
            />*/}
              <span
                className="rc-time-picker xxx"
                style={{ display: "flex", flexWrap: "wrap" }}
              >
                <input
                  className="rc-time-picker-input margin_bottom "
                  placeholder={this.state.placeholderText}
                  value={this.state.selectedText}
                  onChange={e => this.setSelectedText(e.target.value)}
                />
                {this.state.suggestiveButtons.map((item, i) => (
                  <FlatButton
                    key={i}
          ...
          {this.renderSelectedCustomAnimation()}
          {this.renderCustomImage()}
          {this.renderCustomAnimationButton()}
          <div>
            <div style={{ display: "flex", lineHeight: 2, flexWrap: "wrap" }}>
              {/*{this.renderShareTypePopover()}*/}
              <div className="buttons_wrapper">
  ...
  changeShareType = (e, value) => {
    ...
    selectedText =
      value === FACEBOOK_MESSAGE ? "Hey, " + selectedText : selectedText;
  ...
  renderWarningMessage() {
    if (this.state.selectedTime < moment().valueOf()) {
      return (
        <label style={{ margin: "0.5em", fontSize: "12px", color: "#ff9283" }}>
          {WARNING_MESSAGE}
        </label>
      );
    }
          ...
          <Paper className="paper_wrapper" style={{ flex: 1 }}>
            <Calendar
              ...
              dateRender={(current, value) =>
                this.renderCalendarTile(current, value)
              }
            />
          </Paper>
        </div>
  ...
  addNewEvent() {
    console.log('Date here', 'addNewEvent')
    const {
      selectedDate,
      selectedTime,
      shareType,
  ...
  saveEvent = async () => {
    ...
    const segment = this.state.segments.filter(
      item => item.name === selectedSegmentValue
    );
    ...
    if (Object.keys(previewData).length > 1) {
      alert('here', 'First')
      HelperService.renderSVG(previewData, "small").then(svgSting => {


        const imageData = {
          svg: svgSting,
          colors: previewData.animatedColors,
          sizes: { w: OUTPUT_SMALL_POST_WIDTH, h: OUTPUT_SMALL_POST_HEIGHT },
          colorProp: ThemeProvider.getThemes()[previewData.selectedTheme][
            "colorProp"
          ],
          fileName: imageName
        };
      ...
      this.props.cookies.get(PRODUCTS_IN_COOKIE)
      api.uploadImage(this.state.customImage, apiConfig.merchantId).then((res) => {
        if (res.url) {
    ...
    const { cookies } = this.props;
    this.setState({ modalOpened: false, previewData: {} });

    if (
      cookies.get(PRODUCTS_IN_COOKIE) &&
      cookies.get(PRODUCTS_IN_COOKIE).length || this.state.customImage !== null
    )
      this.addNewEvent();
  ...
  removeDate() {
    ...
    this.setState(
      prev => {
        const newArray = prev.selectedDates.filter(
          item => item.date !== this.state.selectedDate
        );
        const newArrayDates = prev.selectedDatesDays.filter(
          item => item !== this.state.selectedDate
        );
...
export default withCookies(
  inject("categoryListStore", "itemListStore", "itemStore")(
    observer(ScheduleScreen)
  )
);
