  ...
  class extends Component {
    ...
    componentWillMount () {
      if (!this.props.categoryListStore.categoryList || this.props.categoryListStore.categoryList.length <= 0) {
        ...
        this.props.categoryListStore.setData({squareAndLocationId: `${Config.merchantId}/${Config.locationId}`});
        this.props.categoryListStore.loadData()
      }
    }

    render () {
      const content = this.props.categoryListStore.fetching
        ? this.renderSpinner()
        : <CategoryHierarchy categoryList={this.props.categoryListStore.categoryList}
                             onCategoryClick={(c) => {
                               this.forceUpdate();
                             }}
                             onItemClick={() => {
